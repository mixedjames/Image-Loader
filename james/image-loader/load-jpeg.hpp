...
namespace james {
  ...
  /**
   * Load a JPEG stream into memory and store it in a `james::Image`.
   *
   * ### Preconditions
   * - src is a valid `std::istream` that points to the beginning of a JPEG stream
   *
   * ### Post-conditions
   * Assuming no exceptions are thrown:
   * - src remains valid and points to just past the end of the JPEG stream.
   * - src remains open
   ...
   *    generated.
   *
   * LoadJPEG provides the basic guarantee: if an exception is thrown, the system will be
   * in a valid state and further calls to LoadJPEG may be made. However, the src stream
